...
class JiraReleaseExtractor:
    ...
    def export_keyword_matches_to_excel(self, df: pd.DataFrame, start_date: str, end_date: str, keywords: str) -> str:
        """
        ...
        Args:
            ...
            start_date (str): Start date for filename
            end_date (str): End date for filename
            keywords (str): Keywords used for search (for filename)
        ...
        try:
            # Create Excel writer with multiple sheets
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                # Main keyword matches sheet
                df.to_excel(writer, sheet_name='Keyword_Matches', index=False)
                
                # Summary sheet by keyword
                keyword_summary = df.groupby(['matched_keyword', 'matched_column']).size().reset_index(name='count')
                keyword_summary.to_excel(writer, sheet_name='Keyword_Summary', index=False)
                
                # Summary sheet by column
                column_summary = df.groupby('matched_column').size().reset_index(name='count')
    ...
    try:
        ...
        if not df.empty:
            ...
            if not filtered_df.empty:
                ...
                keyword_filename = extractor.export_keyword_matches_to_excel(filtered_df, START_DATE, END_DATE, KEYWORDS)
                ...
                summary = filtered_df.groupby(['matched_keyword', 'matched_column']).size().reset_index(name='count')
                for _, row in summary.iterrows():
                    print(f"  - '{row['matched_keyword']}' found {row['count']} times in '{row['matched_column']}'")
            else:
